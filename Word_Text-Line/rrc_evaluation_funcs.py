...
def print_help():
    sys.stdout.write('Usage: python %s.py -g=<gtFile> -s=<submFile> -gl=<gtTextLineFile> [-o=<outputFolder> -p=<jsonParams>]' %sys.argv[0])
    sys.exit(2)
...
def main_evaluation(p,default_evaluation_params_fn,validate_data_fn,evaluate_method_fn,show_result=True,per_sample=True):
    ...
    resDict={'calculated':True,'Message':'','method':'{}','per_sample':'{}', 'tiouMethod': '{}'}    
    try:
        validate_data_fn(p['g'], p['s'], p['gl'], evalParams)  
        evalData = evaluate_method_fn(p['g'], p['s'], p['gl'], evalParams)
